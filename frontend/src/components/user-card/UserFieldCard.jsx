...
export default function UserFieldCard({
  ...
  const [formData, setFormData] = useState({ [fieldKey]: "" });
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setUser(userData);
        setFormData({ [fieldKey]: userData[fieldKey] || "" });
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = endpoint.replace(":id", user.id);
      await api.patch(url, { [fieldName]: formData[fieldKey] });

      Swal.fire("Success", `${title} updated successfully!`, "success");
      setIsEditing(false);
    // eslint-disable-next-line no-unused-vars
    } catch (error) {
      Swal.fire("Failed", "Failed to update field.", "error");
    } finally {
              ...
              <div className="mt-4 space-y-4 max-w-md">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({ [fieldKey]: e.target.value })
                    }
